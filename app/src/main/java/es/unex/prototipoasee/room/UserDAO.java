package es.unex.prototipoasee.room;

import androidx.room.Dao;
import androidx.room.Delete;
import androidx.room.Insert;
import androidx.room.Query;

import java.util.List;

import es.unex.prototipoasee.model.User;

@Dao
public interface UserDAO {

    // Inserta un usuario en la BD
    @Insert
    void insertUser(User user);

    // Elimina un usuario de la BD
    @Delete
    void deleteUser(User user);

    // Elimina un usuario de la BD dado su nombre de usuario
    @Query("DELETE FROM Users WHERE username = (:username)")
    void deleteUserByID(String username);

    // Devuelve un usuario dado su nombre de usuario
    @Query("SELECT * FROM Users WHERE username = (:username)")
    User getUser(String username);

    // Devuelve una lista con todos los usuarios almacenados
    @Query("SELECT * FROM Users")
    List<User> getAllUsers();

    // Devuelve 1 si existe el usuario con las credenciales introducidas
    @Query("SELECT 1 FROM Users WHERE username = (:username) AND password = (:password)")
    int checkUserCredentials(String username, String password);

    // Devuelve 1 si existe un usuario con el nombre de usuario indicado
    @Query("SELECT 1 FROM Users WHERE username = (:username)")
    int checkIfUsernameExists(String username);

    // Devuelve 1 si existe un usuario con el email indicado
    @Query("SELECT 1 FROM Users WHERE email = (:email)")
    int checkIfEmailExists(String email);
}
